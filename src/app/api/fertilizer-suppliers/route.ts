...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/fertilizer-suppliers`, {
      method: 'GET',
      headers,
      cache: 'no-store',
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || errorData.error || 'Failed to fetch fertilizer suppliers' },
    ...
    console.error('Error fetching fertilizer suppliers:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { error: errorData.message || 'Failed to create fertilizer supplier' },
        { status: response.status }
      );
    }
    ...
    console.error('Error creating fertilizer supplier:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
