...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/fertilizer-suppliers/${id}`, {
      method: 'GET',
      headers,
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || errorData.error || 'Failed to fetch fertilizer supplier' },
    ...
    console.error('Error fetching fertilizer supplier:', error);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { error: errorData.message || 'Failed to update fertilizer supplier' },
        { status: response.status }
      );
    }
    ...
    console.error('Error updating fertilizer supplier:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: 'Failed to delete fertilizer supplier' },
        { status: response.status }
      );
    }
    ...
    console.error('Error deleting fertilizer supplier:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
