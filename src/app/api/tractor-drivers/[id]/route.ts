...
export async function GET(
  ...
  try {
    ...
    console.error('Error fetching tractor driver:', error);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Failed to update tractor driver' },
        { status: response.status }
      );
    }
    ...
    console.error('Error updating tractor driver:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: 'Failed to delete tractor driver' },
        { status: response.status }
      );
    }
    ...
    console.error('Error deleting tractor driver:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
